...
parser.add_argument('--model_path', default='/media/dawei-server/DATA - HDD8TB/wyc/数据保存/dgcnn/3DEPSratio/average/198_3DEPS0.25-2/epoch_198.ckpt',
                    help='model checkpoint file path [default: log/model.ckpt]')
parser.add_argument('--log_dir', default='Epoch99', help='Log dir [default: log_eval]')
...
test_sem = []
for h5_filename_test in test_file_list:
    cur_data,cur_sem= provider.load_h5_data_label_seg1(os.path.join(ROOT_DIR, h5_filename_test))
    test_data.append(cur_data)
    test_sem.append(cur_sem)
...
def eval_one_epoch(sess, ops):
    ...
    seg_classes = {'Bed':[i for i in range(0,10)],'Chair':[i for i in range(10,40)],'Dishwasher':[i for i in range(40,45)],'Door':[i for i in range(45,49)],'Lamp':[i for i in range(49,77)],
                   'Microwave':[i for i in range(77,82)],'Refrigerator':[i for i in range(82,88)],'StorageFurniture':[i for i in range(88,107)],'Table':[i for i in range(107,149)]}
