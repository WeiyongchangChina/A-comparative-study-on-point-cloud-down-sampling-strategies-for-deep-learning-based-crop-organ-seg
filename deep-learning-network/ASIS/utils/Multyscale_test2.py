...
# -*- coding: utf-8 -*-
"""
Created on Thu Jul 18 13:22:08 2019
...
def feature_nearst(feature_data,points_xyz,nsample_point,mlp1,mlp2,scope,is_training,bn_decay,radius=None,bn=True):
    ...
    with tf.variable_scope(scope) as sc:
        ...
        net_ins1 = tf_util.conv1d(feature_points, 3, 1, padding='VALID', bn=True, is_training=is_training, scope='first_feature1', bn_decay=bn_decay)
        net_ins1 = tf_util.dropout(net_ins1, keep_prob=0.5, is_training=is_training, scope='first_feature2')
        net_ins1 = tf_util.conv1d(net_ins1, 5, 1, padding='VALID', activation_fn=None, scope='first_feature3')#B X N X 3
        # pooling in globle feature
        net_globle = tf.reduce_max(feature_points, axis=[1], keep_dims=True, name='maxpool2')#Bx1x3
        # concat
        net_globle = tf.tile(net_globle, [1, 4096, 1])
        net_globle_concat0 = tf.concat(axis=-1, values=[feature_data, feature_points])# (batch_size, npoints, 128+3+3)
        net_globle_concat1 = tf.concat(axis=-1, values=[net_globle_concat0, net_globle])# (batch_size, npoints, 3+128+3+5)
        #Grouping
        net_group = sample_and_group(radius, nsample_point, feature_points, net_globle_concat1, knn=True, use_xyz=False)#B X N x 32 x 139
        #point feature embedding
        ...
        # Pooling in Local Regions
        net_group = tf.reduce_max(net_group, axis=[2], keep_dims=True, name='maxpool')#B X N x 1 x 32
        net_group = tf.squeeze(net_group, [2])#B X N x 32
#        return tf.concat([points_xyz, net_group], axis=-1)#B X N x 32+3
