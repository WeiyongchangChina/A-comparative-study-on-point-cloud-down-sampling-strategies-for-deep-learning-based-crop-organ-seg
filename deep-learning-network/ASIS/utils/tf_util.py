...
def pairwise_distance(point_cloud):
  ...
  point_cloud_transpose =tf.transpose(point_cloud, [0, 2, 1])
  point_cloud_inner = tf.matmul(point_cloud, point_cloud_transpose)
  point_cloud_inner = -2*point_cloud_inner
  point_cloud_square = tf.expand_dims(tf.reduce_sum(tf.square(point_cloud), -1),-1)
  point_cloud_square_tranpose = tf.transpose(point_cloud_square,[0, 2, 1])
...
def knn_thres(adj_matrix, k=20, thres=0.5):
  ...
  to_add = tf.range(nn_idx.get_shape()[1])#[0 1 2 ... 4095]
  to_add = tf.reshape(to_add, [-1, 1])#shape:4096 x 1
  to_add = tf.tile(to_add, [1, k]) #shape: 4096 x k

  final_nn_idx = []
  for idx in range(og_batch_size):
    idx_vals = vals[idx, :, :]
    idx_nn_idx = nn_idx[idx, :, :]
    mask = tf.cast(idx_vals < -1*thres, tf.int32) # [N, K] distance large than 0.5 equal 1, else 0
