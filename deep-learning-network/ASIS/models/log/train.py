...
parser.add_argument('--input_list_train', type=str, default='/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/ASIS_plant_master_temp1/data/train_file_list.txt', help='Input data list file')
parser.add_argument('--input_list_test', type=str, default='/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/ASIS_plant_master_temp1/data/test_file_list.txt', help='Input data list file')
...
def train_one_epoch(sess, ops, train_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        start = time.clock()
        summary, step, _, loss_val, sem_loss_val, disc_loss_val, l_var_val, l_dist_val, l_reg_val, pred_val = sess.run([ops['merged'], ops['step'], ops['train_op'], ops['loss'], ops['sem_loss'], ops['disc_loss'], ops['l_var'], ops['l_dist'], ops['l_reg'], ops['pred_sem']],
                                         feed_dict=feed_dict)
        global sum
