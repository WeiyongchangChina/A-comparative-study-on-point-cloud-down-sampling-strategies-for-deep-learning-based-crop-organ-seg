...
parser.add_argument('--input_list', type=str, default='/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/ASIS_plant_master/data/test_file_list.txt', help='Input data list file')
parser.add_argument('--model_path', type=str, default='/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/ASIS_plant_master_temp1/models/log/epoch_199.ckpt', help='Path of model')
...
mean_num_pts_in_group = np.loadtxt(os.path.join(MODEL_PATH.split('/')[0], '/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/ASIS_plant_master/models/mean_ins_size.txt'))
...
OUTPUT_DIR = os.path.join(LOG_DIR, 'new_data_1')
...
#        total_seen = 0
        ...
        for shape_idx in range(len_pts_files):
            ...
            # cur_data = cur_data[:,index_test,:]
            # cur_group = cur_group[:,index_test]
            # cur_sem = cur_sem[:,index_test]
            # cur_obj = cur_obj[:,index_test]
