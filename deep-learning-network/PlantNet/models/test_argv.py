...
ROOT_DIR = '/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/PlantNet_temp2/'
...
parser.add_argument('--input_list', type=str, default='/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/PlantNet_temp2/data/test_file_list.txt', help='Input data list file')
parser.add_argument('--model_path', type=str, default='/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/PlantNet_temp2/models/log_test/epoch_100.ckpt', help='Path of model')
FLAGS = parser.parse_args(args=[]) #改了
...
mean_num_pts_in_group = np.loadtxt("/media/dawei-server/DATA - SSD1TB/wyc/服务器训练/data_sgl/data_net_sgl/PlantNet_temp2/models/mean_ins_size.txt")#loadtxt(os.path.join(MODEL_PATH.split('/')[5], '../mean_ins_size.txt'))
...
#        total_seen = 0
        ...
        for shape_idx in range(len_pts_files):
            ...
            for j in range(num_data):
                ...
                pts_label_one_hot, pts_label_mask = convert_seg_to_one_hot(cur_sem[j:j+1])
                ...
                loss_val, l_var_val, l_dist_val, l_reg_val, pred_ins_val, pred_sem_label_val, pred_sem_softmax_val = sess.run(
                    ...
                    feed_dict=feed_dict)
                # np.save("/media/david/HDD1/sgl/ASIS_DGCNN_TEST_V2/models/out/log_6_BS74_0.01_test1/pred_ins_val.npy",pred_ins_val)
                # np.save("/media/david/HDD1/sgl/ASIS_DGCNN_TEST_V2/models/out/log_6_BS74_0.01_test1/sem.npy",sem)
                # np.save("/media/david/HDD1/sgl/ASIS_DGCNN_TEST_V2/models/out/log_6_BS74_0.01_test1/group.npy",group)
                # np.save("/media/david/HDD1/sgl/ASIS_DGCNN_TEST_V2/models/out/log_6_BS74_0.01_test1/pre_ins.npy",group_pred_final)
