...
def my_dgcnn(new_points, k,d,num_out_channel,is_training,scope,bn_decay):
    ...
    edge_feature = tf_util.get_edge_feature(new_points, nn_idx=nn_idx, k=k)  # [N,k,2*d]

    # net = tf_util.conv2d(edge_feature, num_out_channel, [1, edge_feature.get_shape()[2].value],
    ...
    with tf.variable_scope(scope) as sc:
        ...
        if mlp1 is not None:
            ...
            for i, num_out_channel in enumerate(mlp1):
                # if i==0 or i==1:
                #     continue
                # _,neigh_idx=knn_point(k,xyz,cur_points[:,:,:3])
                # new_points1=relative_pos_encoding( cur_points, neigh_idx)
                adj_matrix = tf_util.pairwise_distance(new_points)  # [4096,4096]
                nn_idx = tf_util.dg_knn(adj_matrix, k=k, d=d)
                ...
                #                             padding='VALID', stride=[1,1],
                #                             bn=bn, is_training=is_training,
                #                             scope='lfa%d'%(i), bn_decay=bn_decay,
                #                             data_format=data_format)
